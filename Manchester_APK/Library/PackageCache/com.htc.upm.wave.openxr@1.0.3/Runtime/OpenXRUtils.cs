...
namespace Wave.OpenXR
{
    ...
    /// 
    /// <see cref="OpenXRHelper.XR_SPACE_LOCATION_ORIENTATION_VALID_BIT">XR_SPACE_LOCATION_ORIENTATION_VALID_BIT</see> indicates that the pose field’s orientation field contains valid data. For a space location tracking a device with its own inertial tracking, XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT should remain set when this bit is set. Applications must not read the pose field’s orientation if this flag is unset.
    /// 
    /// <see cref="OpenXRHelper.XR_SPACE_LOCATION_POSITION_VALID_BIT">XR_SPACE_LOCATION_POSITION_VALID_BIT</see> indicates that the pose field’s position field contains valid data. When a space location loses tracking, runtimes should continue to provide valid but untracked position values that are inferred or last-known, so long as it’s still meaningful for the application to use that position, clearing XR_SPACE_LOCATION_POSITION_TRACKED_BIT until positional tracking is recovered. Applications must not read the pose field’s position if this flag is unset.
    /// 
    /// <see cref="OpenXRHelper.XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT">XR_SPACE_LOCATION_ORIENTATION_TRACKED_BIT</see> indicates that the pose field’s orientation field represents an actively tracked orientation. For a space location tracking a device with its own inertial tracking, this bit should remain set when XR_SPACE_LOCATION_ORIENTATION_VALID_BIT is set. For a space location tracking an object whose orientation is no longer known during tracking loss (e.g. an observed QR code), runtimes should continue to provide valid but untracked orientation values, so long as it’s still meaningful for the application to use that orientation.
    /// 
    /// <see cref="OpenXRHelper.XR_SPACE_LOCATION_POSITION_TRACKED_BIT">XR_SPACE_LOCATION_POSITION_TRACKED_BIT</see> indicates that the pose field’s position field represents an actively tracked position. When a space location loses tracking, runtimes should continue to provide valid but untracked position values that are inferred or last-known, e.g. based on neck model updates, inertial dead reckoning, or a last-known position, so long as it’s still meaningful for the application to use that position.
    /// </summary>
